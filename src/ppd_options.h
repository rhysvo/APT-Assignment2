...
/** 
 * Defines the I in the id, so that the tokenizer can get the number in the ID.
 **/
#define ID_DELIM "I"

/**
 * Defines the "." in the price, so that the value for dollars and cents can be
 * saved separatley in the stock struct.
 **/
#define CURRENCY_DELIM "."
...
/**
 * This option will procceed to free all heap memory allocated by the program, 
 * and call the exit(EXIT_SUCCESS) function, without saving the changes to the 
 * linked list to file
 **/
